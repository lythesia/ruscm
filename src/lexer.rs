...
impl<'a> Lexer<'a> {
    ...
    fn parse_boolean(&mut self) -> Result<bool, LexError> {
        match self.current {
            Some('t') => Ok(true),
            Some('f') => Ok(false),
            Some(c) => lex_error!(self, "unexpected character: `{}', looking for boolean(t or f)", c),
            _ => lex_error!(self, "unexpected eof"),
        }
    }
    ...
    fn parse_character(&mut self) -> Result<char, LexError> {
        match self.current {
            Some('\\') => {
                let s = peek_while(&mut self.chars, |c: &char| c.is_alphabetic()).collect::<String>();
                match s.as_ref() {
...
//                self.advance();
    ...
    fn parse_radix(&mut self) -> Result<u32, LexError> {
        match self.current {
            ...
            Some('d') => Ok(10),
            Some('x') => Ok(16),
            Some(c) => lex_error!(self, "unexpected character: `{}', looking for radix symbol", c),
            _ => lex_error!(self, "unexpected eof"),
        }
    }
    ...
    fn parse_integer(&mut self, radix: u32) -> Result<i64, LexError> {
        ...
        let s = if let Some(c) = sign {
            let mut s = peek_while(&mut self.chars, |c: &char| c.is_digit(radix)).collect::<String>();
            s.insert(0, c);
            s
        } else {
    ...
    fn parse_float(&mut self) -> Result<f64, LexError> {
        ...
        let s = if let Some(sgn) = sign {
            let mut s = peek_while(&mut self.chars, |c: &char| !Self::is_delimiter(c)).collect::<String>();
            s.insert(0, sgn);
            s
        } else {
            // current is digit now
                    ...
                    match c {
                        ...
                        '.' => {
                            ...
                            if let Some('.') = self.peek() {
                                let s = peek_while(&mut self.chars, |c| c == &'.').collect::<String>();
                                if s != ".." {
                                    lex_error!(self, "unexpected sequence: `{}', looking for ...", s);
                                } else {
...
mod tests {
    ...
    fn test_lex_simple() {
        assert_eq!(
            ...
            vec![Token::LPAREN, Token::IDENTIFIER("+".to_string()), Token::INTEGER(11), Token::INTEGER(-234), Token::RPAREN]
        );
    }
    ...
    fn test_lexer_delim() {
        ...
        assert_eq!(
            tokenize("(cons #\\a #\\newline )").unwrap(),
            vec![Token::LPAREN, Token::IDENTIFIER("cons".to_string()), Token::CHARACTER('a'), Token::CHARACTER('\n'), Token::RPAREN]
        );
    }
    ...
    fn test_lexer_whitespace() {
        assert_eq!(tokenize("(+ 1 1)\n(+\n    2\t+2 \n )\r\n  \n").unwrap(),
                   vec![Token::LPAREN, Token::IDENTIFIER("+".to_string()), Token::INTEGER(1), Token::INTEGER(1), Token::RPAREN,
                        Token::LPAREN, Token::IDENTIFIER("+".to_string()), Token::INTEGER(2), Token::INTEGER(2), Token::RPAREN]);
    }
    ...
    fn test_lexer_quoting() {
        assert_eq!(tokenize("'(a)").unwrap(),
                   vec![Token::QUOTE, Token::LPAREN, Token::IDENTIFIER("a".to_string()), Token::RPAREN]);
        assert_eq!(tokenize("'('a 'b)").unwrap(),
                   vec![Token::QUOTE, Token::LPAREN, Token::QUOTE, Token::IDENTIFIER("a".to_string()), Token::QUOTE, Token::IDENTIFIER("b".to_string()), Token::RPAREN]);
        assert_eq!(tokenize("(list 'a b)").unwrap(),
                   vec![Token::LPAREN, Token::IDENTIFIER("list".to_string()), Token::QUOTE, Token::IDENTIFIER("a".to_string()), Token::IDENTIFIER("b".to_string()), Token::RPAREN]);
    }
    
    #[test]
    fn test_lexer_quasiquoting() {
        assert_eq!(tokenize("`(,a)").unwrap(),
                   vec![Token::QUASIQUOTE, Token::LPAREN, Token::UNQUOTE, Token::IDENTIFIER("a".to_string()), Token::RPAREN]);
        assert_eq!(tokenize("`(,a b ,c)").unwrap(),
                   vec![Token::QUASIQUOTE, Token::LPAREN, Token::UNQUOTE, Token::IDENTIFIER("a".to_string()), Token::IDENTIFIER("b".to_string()), Token::UNQUOTE, Token::IDENTIFIER("c".to_string()), Token::RPAREN]);
    }
    
    #[test]
    fn test_vardiac_arg() {
        assert_eq!(tokenize("(lambda (x y . z))").unwrap(),
                   vec![Token::LPAREN, Token::IDENTIFIER("lambda".to_string()),
                        Token::LPAREN, Token::IDENTIFIER("x".to_string()), Token::IDENTIFIER("y".to_string()), Token::DOT, Token::IDENTIFIER("z".to_string()),
    ...
    fn test_lexer_unicode_identifiers() {
        assert_eq!(tokenize("λ").unwrap(), // alphabetic
    ...
    fn test_lexer_reserved() {
        ...
        println!("{}", tokenize("(cons \"abc\" #\\c \\b)").err().unwrap().to_string());
    }
}